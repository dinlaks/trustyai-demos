...
This demo leverages [KServe](https://github.com/kserve/kserve) for model deployment. 
...
## Setup
Follow the instructions within the [Installation section](1-Installation/README.md). Afterwards, you should have OpenShift AI installation, a TrustyAI Operator, and a `model-bias` project containing an instance of the TrustyAI Service.
...
## Deploy Models
1) Navigate to the `model-bias` created in the setup section: `oc project model-bias`
...
6) From the OpenShift Console, navigate to the `model-bias` project and look at the Workloads -> Pods screen.

To sense-check that the namespace contains the expected resources, navigate to the OpenShift Console and look at the Workloads -> Pods screen. You should see the following [pods](images/model_bias_pods.png):
