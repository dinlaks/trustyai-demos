...
## Setup
Follow the instructions within the [Installation section](1-Installation/README.md). Afterwards, you should have an OpenShift AI installation, a TrustyAI Operator, and a `model-drift` project containing an instance of the TrustyAI Service.
...
## Deploy Model
1) Navigate to the `model-drift` created in the setup section
2) Deploy the model's storage container
3) Deploy the Seldon MLServer serving runtime
4) Deploy the credit model
6) From the OpenShift Console, navigate to the `model-drift` project and look at the Workloads -> Pods screen. You should see the following pods within the previously created [namespace](images/model_drift_pods.png)
